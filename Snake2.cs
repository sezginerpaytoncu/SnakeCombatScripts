...
public class Snake2 : MonoBehaviour
{
   ...
   private void Awake()
    ...
    private void Update()
    {
        ...
        if (snake2Dead == false&&snake3Dead==false)
        {
            HandleInput();
            HandleGridMovement();
            MoveBodyParts();
    ...
    {
        if (Input.GetKeyDown(KeyCode.W) && gridMoveDirection.y != -1 && antiBug!=gridPosition)
        {
        ...
        if (Input.GetKeyDown(KeyCode.S) && gridMoveDirection.y != 1 && antiBug != gridPosition)
        {
            gridMoveDirection.x = 0;
            gridMoveDirection.y = -1;
        ...
        if (Input.GetKeyDown(KeyCode.A) && gridMoveDirection.x != 1 && antiBug != gridPosition)
        {
        ...
        if (Input.GetKeyDown(KeyCode.D) && gridMoveDirection.x != -1 && antiBug != gridPosition)
        {
            gridMoveDirection.x = 1;
            gridMoveDirection.y = 0;
            transform.rotation = Quaternion.Euler(0, 0, -90);
